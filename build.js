...
const esbuild = await (async () => {
    // Prefer the native esbuild whenever its platform binary actually runs —
    // it is much faster and more reliable than the wasm build. Fall back to
    // esbuild-wasm (arches without a native binary installed), then to the
    // distro package (e.g. Debian's /usr/lib/*/nodejs/esbuild).
    for (const pkg of ['esbuild', 'esbuild-wasm']) {
        try {
            const mod = (await import(pkg)).default;
            // Importing succeeds even without a matching platform binary;
            // run a no-op API call to verify the binary really works.
